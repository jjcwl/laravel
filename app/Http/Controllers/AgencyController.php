	...
	*/
	public function index(Request $request){
		...
		foreach ($url as $value) {
			...
			//检测当前访问的路由是否在权限当中
			if(in_array($b, $urll)){
				$list = DB::table('agency')->orderBy('create_time','desc')->paginate(10);
				$list->setPath('index');
				return view('agency/index',['list'=>$list]);
			}
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				if($request->input('sub')){
					...
					$data=DB::table('agency')->insert([
						'id'=>$id,'describe'=>$describe,
						'logo'=>$filename,'create_uid'=>$create_uid,
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				if($request->input('sub')){
					...
					$file = $request->file('logo');
					if($file['0']==""){ 
						$data=DB::table('agency')->where('id',$hid)->update([
						'describe'=>$describe,
						...
						//删除图片
						$list=DB::table('agency')->where('id',$hid)->first();
						$photo=$list->logo;
						if($photo){
							$photo=explode(',',$photo);
						...
						$data=DB::table('agency')->where('id',$hid)->update([
						'describe'=>$describe,
						'logo'=>$filename,'update_uid'=>$create_uid,
						'update_time'=>$create_time
						]);	
					}
					if($data){
					...
					$id=$request->input('id');
					$list=DB::table('agency')->where('id',$id)->first();
					$photo=$list->logo;
					$photos=explode(',',$photo);
					return view('agency/mod',['list'=>$list,
						'photos'=>$photos]);
				}
			}
	...
	public function del(Request $request){
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				...
				$data=DB::table('agency')->where('id',$id)->delete();
