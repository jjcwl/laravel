...
class HotController extends Controller{	
	...
	public function index(Request $request){
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				...
				$list = DB::table('hot')->paginate(10);
				$list->setPath('index');
				return view('hot/index',['list'=>$list]);
			}
		}
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				if($request->input('sub')){
					...
					$name=$request->input('name');
					$list=DB::table('hot')->where('pid',$hid)->update([
					...
					$id=$request->input('id');
					$list=DB::table('hot')->where('pid',$id)->first();
					return view('hot/mod',['list'=>$list]);
				}
			}
		}
		return "<script>alert('您没有权限访问');location.href='".$_SERVER["HTTP_REFERER"]."';</script>";
	...
	public function del(Request $request){
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				$id=$request->input('id');
				$data=DB::table('hot')->where('pid',$id)->delete();
				if($data){
