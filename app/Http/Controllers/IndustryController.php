...
class IndustryController extends Controller{	
	...
	public function index(Request $request){
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				...
				$list = DB::table('industry')->paginate(10);
				$list->setPath('index');
				return view('industry/index',['list'=>$list]);
			}
		}
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				...
				if($request->input('sub')){
					...
					$data=DB::table('industry')->insert([
						'iname'=>$iname,'creates_uid'=>$create_uid,
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				if($request->input('sub')){
					...
					$iname=$request->input('iname');
					$list=DB::table('industry')->where('iid',$hid)->update([
						'iname'=>$iname,'updates_uid'=>$update_uid,
					...
					$id=$request->input('id');
					$list=DB::table('industry')->where('iid',$id)->first();
					return view('industry/mod',['list'=>$list]);
				}
			}
		}
		return "<script>alert('您没有权限访问');location.href='".$_SERVER["HTTP_REFERER"]."';</script>";
	...
	public function del(Request $request){
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				$id=$request->input('id');
				$data=DB::table('industry')->where('iid',$id)->delete();
				if($data){
