...
class SourceController extends Controller{	
	...
	public function index(Request $request){
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				...
				$list = DB::table('source')->paginate(10);
				$list->setPath('index');
				return view('source/index',['list'=>$list]);
			}
		}
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				...
				if($request->input('sub')){
					...
					$data=DB::table('source')->insert([
						'sname'=>$sname,'createso_uid'=>$create_uid,
						'updateso_uid'=>$create_uid,'createso_time'=>$create_time,
						'updateso_time'=>$create_time
						]);
					if($data){
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				if($request->input('sub')){
					...
					$sname=$request->input('sname');
					$list=DB::table('source')->where('sid',$hid)->update([
						'sname'=>$sname,'updateso_uid'=>$update_uid,
						'updateso_time'=>$update_time
						]);
					if($list){
					...
					$id=$request->input('id');
					$list=DB::table('source')->where('sid',$id)->first();
					return view('source/mod',['list'=>$list]);
				}
			}
		}
		return "<script>alert('您没有权限访问');location.href='".$_SERVER["HTTP_REFERER"]."';</script>";
	...
	public function del(Request $request){
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				$id=$request->input('id');
				$data=DB::table('source')->where('sid',$id)->delete();
				if($data){
