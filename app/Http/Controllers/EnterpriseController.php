	...
	*/
	public function index(Request $request){
		...
		foreach ($url as $value) {
			...
			//检测当前访问的路由是否在权限当中
			if(in_array($b, $urll)){
				$list = DB::table('enterprise')
						->join('industry','enterprise.industry','=',
							'industry.iid')
						->orderBy('create_time','desc')
						->paginate(10);
				$list->setPath('index');
				return view('enterprise/index',['list'=>$list]);
			}
		}
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				if($request->input('sub')){
					...
					return view('enterprise/add',['list'=>$list,'cooperation'=>$cooperation]);
				}
			}
		}
		return "<script>alert('您没有权限访问');location.href='".$_SERVER["HTTP_REFERER"]."';</script>";	
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				if($request->input('sub')){
					...
					$file = $request->file('logo');
					if($file['0']==""){ 
						$data=DB::table('enterprise')->where('id',$hid)->update([
						'name'=>$name,'introduction'=>$introduction,
						'field'=>$field,'industry'=>$industry,
						'contact'=>$contact,'phone'=>$phone,
						'en_project'=>$en_project,'area'=>$area,
						'cooperation'=>$cooperation,'money'=>$money,
						...
						//删除图片
						$list=DB::table('enterprise')->where('id',$hid)->first();
						$photo=$list->logo;
						if($photo){
							$photo=explode(',',$photo);
					...
					$id=$request->input('id');
					$list=DB::table('enterprise')->where('id',$id)->first();
					$data=DB::table('industry')->get();
					$cooperation=DB::table('cooperation')->get();
					$photo=$list->logo;
					$photos=explode(',',$photo);
					return view('enterprise/mod',['list'=>$list,'data'=>$data,
						'photos'=>$photos,'cooperation'=>$cooperation,]);
	...
	public function del(Request $request){
		...
		foreach ($url as $value) {
			...
			if(in_array($b, $urll)){
				...
				$data=DB::table('enterprise')->where('id',$id)->delete();
